...
You can also set the text shown after expanding (default "Свернуть") and choose the button alignment. Hover effects for rows and links use speeds defined on this page so transitions match your design. The style settings also allow defining a price suffix (e.g. "BYN") that appears after every price value.
